...
namespace bitbot
{
  JointElmoOld::JointElmoOld(const pugi::xml_node& motor_node)
    : CifxJoint(motor_node)
  {
    basic_type_ = static_cast<uint32_t>(BasicDeviceType::MOTOR);
    type_ = static_cast<uint32_t>(CifxDeviceType::JOINT_ELMO_OLD);
    input_data_length_ = 16;
    output_data_length_ = 30;
  ...
  void JointElmoOld::InitParams()
  {
    joint_data_.config.current_ratio = 10000;
    PowerOff();  //0x06
    SetMode(static_cast<CANopenMotorMode>(joint_data_.config.mode));
    SetTargetVelocity(0);
    // SetProfileVelocity(100 * joint_data_.transmission_ratio * joint_data_.encoder_resolution);
    // SetAcceleration(50 * joint_data_.transmission_ratio * joint_data_.encoder_resolution);
    // SetDeceleration(50 * joint_data_.transmission_ratio * joint_data_.encoder_resolution);
    SetProfileVelocity(2e9);
    SetAcceleration(2e9);
    SetDeceleration(2e9);
    SetEndVelocity(0);
    SetTargetTorque(0);
  ...
  JointPowerOnState JointElmoOld::PowerOn()
  {
    ...
    // logger_->debug("poweron step: {}", poweron_step_);
    // 位置参数复位
  ...
  void JointElmoOld::SetTargetPosition(double position)
  {
  ...
  void JointElmoOld::SetTargetVelocity(double velocity)
  {
    static int32_t count = 0;
  ...
  void JointElmoOld::SetTargetTorque(double torque)
  {
  ...
  void JointElmoOld::SetTargetCurrent(double current)
  {
  ...
  void JointElmoOld::SetProfileVelocity(uint32_t profile_velocity)
  {
    uint32_t bus_send;

    bus_send = profile_velocity;
    memcpy(&bus_send_data_[elmo_send_addr.profile_velocity], &bus_send, sizeof(uint32_t));
  }

  void JointElmoOld::SetEndVelocity(uint32_t end_velocity)
  {
    uint32_t bus_send;

    bus_send = end_velocity;
    memcpy(&bus_send_data_[elmo_send_addr.end_velocity], &bus_send, sizeof(uint32_t));
  }
  void JointElmoOld::SetAcceleration(uint32_t acceleration)
  {
    uint32_t bus_send;

    bus_send = acceleration;
    memcpy(&bus_send_data_[elmo_send_addr.acceleration], &bus_send, sizeof(uint32_t));
  }
  void JointElmoOld::SetDeceleration(uint32_t deceleration)
  {
    uint32_t bus_send;

    bus_send = deceleration;
    memcpy(&bus_send_data_[elmo_send_addr.deceleration], &bus_send, sizeof(uint32_t));
  }

  void JointElmoOld::UpdateRuntimeData()
  {
    constexpr double rad2deg = 180.0/M_PI;
    // "status","mode","actual_position","target_position","actual_velocity","target_velocity","actual_current","target_torque"
